...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach (['image1', 'image2', 'image3', 'image4'] as $index => $imageKey) {
            if ($request->hasFile($imageKey)) {
                $image = $request->file($imageKey);
                $imageName = time() . uniqid('_' . ($index + 1) . '_') . $image->getClientOriginalName();
                $image->move($imagePath, $imageName);
                $validatedData["image_url" . ($index + 1)] = $imagePath . "/" . $imageName;
            }
        }
    ...
    //     ProcessProductImages::dispatch($request, $product, $imagePath);
