        ...
        <div class="row p-4">
            ...
            <div class="col-md-4 d-flex flex-column justify-content-end align-items-end mb-3">
                <div class="btn-group" role="group" aria-label="Basic example">
                    ...
                    <form action="{{ route('savelater.remove') }}" method="POST" onsubmit="showLoader(this)">
                        @csrf
                        <input type="hidden" name="product_id" value="{{ $savedItem->deal_id }}">
                        <button type="submit" class="btn border btn-outline-secondary"
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        orderNowButton.addEventListener('click', (event) => {
            ...
            document.querySelectorAll('.service-date').forEach((serviceDateField) => {
                ...
                const serviceTimeField = document.querySelector(`.service-time[data-product-id="${productId}"]`);
                ...
                if (!serviceTimeField.value) {
                    ...
                    if (selectedDate.toDateString() === currentDate.toDateString()) {
                        ...
                        if (inputHours < currentHours || (inputHours === currentHours && inputMinutes < currentMinutes)) {
                            valid = false;
                            displayError(serviceTimeField, 'Service Time must be in the future');
                            if (!firstInvalidField) firstInvalidField = serviceTimeField;
                        }
                    }
                }
            ...
            if (!valid) {
                ...
                document.querySelectorAll('.service-date').forEach((serviceDateField) => {
                    ...
                    const serviceTime = document.querySelector(`.service-time[data-product-id="${productId}"]`).value;
            ...
            .then((data) => {
                if (data.status === 'success') {
                    ...
                    document.querySelectorAll('.discount').forEach((element) => {
                        element.textContent = `₹${data.updatedCart.discount.toFixed(2)}`;
                    });
